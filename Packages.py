...
class qt(Package):
    ...
    chksum  = 'e1e2edef1d63ed677d6534d32800c2e1f7ad0e73'
    patches = 'patches/qt'
    patch_level = '-p0'
    ...
    @stage
    def configure(self):
        cmd = './configure -opensource -fast -confirm-license -nomake demos -nomake examples -nomake docs -nomake translations -no-webkit -prefix %(INSTALL_DIR)s -no-script -no-scripttools -no-openssl -no-libjpeg -no-libmng -no-libpng -no-libtiff -no-cups -no-nis -no-opengl -no-openvg' % self.env
        args = cmd.split()
        if self.arch.os == 'osx':
